...
Black Stone FX trading education website — a single-page marketing site for a trading education brand. Built with TanStack Start and deployed on Netlify. The site showcases three learning pillars (Technical Analysis, Fundamental Analysis, Risk Management), free activities (Webinars, Competition, WhatsApp Community), and a VIP group offering.

## Brand & Design

- **Color palette**: Black (#0a0a0a) background with gold (#c9a84c / #f0c75e) accents
- **Typography**: Cinzel (serif, Google Fonts) for headings; Inter for body
- **Aesthetic**: Luxury dark gold trading brand matching the uploaded brand assets
- **Images**: All brand images in `public/images/` (brandbanner, technical, fundamental, riskmanagement, vip, community, competition, Webinar)

## Social Links

All social/WhatsApp links are constants at the top of `src/routes/index.tsx`. Update these before going live:
- `WHATSAPP_COMMUNITY` — Free community group invite link
- `WHATSAPP_VIP` — VIP group invite link  
- `INSTAGRAM`, `YOUTUBE`, `TELEGRAM`

## Non-obvious Decisions

- All sections are in a single `index.tsx` — no sub-routes needed for a one-page site
- Custom CSS classes in `styles.css` for gold gradients, ticker tape animation, float animation
- Cinzel font loaded via Google Fonts link tags in `__root.tsx` head
- Ticker tape uses CSS `animation: ticker` with duplicated content for seamless infinite loop
- `foo.css` in project root is a scaffold artifact — can be ignored
